...
function removeItem(listArray, item){
  if(!item){
    return []
  }
  if(!listArray){
...
function addItem(listArray, item){
  let itsHere = false
  if(!item){
    return []
  }
...
function addTen(numbers) {
  ...
  for(i=0; i< numbers.length; i++) {
      ...
      const plusTen = Number(numbers[i]) + 10
      numbers.splice(i, 1, plusTen)
...
console.log(removeItems(devMountainEmployees, "Cahlan"))
...
var user2 = {
  name: 'Frank Severs',
  email: 'Frank.Severs@devmounta.in',
  password: 'armslength122',
  username: 'frank.severs'
};

var user3 = {
  name: 'Patricia Weaver',
  email: 'Patricia.Weaver@devmounta.in',
...
console.log(removeUser(users, "mark.mciver@devmounta.in"))
/*
